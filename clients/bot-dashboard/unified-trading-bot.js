...
// resolution (previously ~200 lines of try/require/stub chains here) lives in
// stock-modules.manifest.js, loaded once at boot by shared/module-loader.js.
...
const moduleRegistry = loadModules(require('./stock-modules.manifest'), { baseDir: __dirname });
let {
    ...
    crossAssetModule, microstructureModule, liquiditySweepModule,
    initSignalSchema, vwapReversalStrategy, MonteCarloSizer,
...
app.get('/api/admin/engine-credentials', requireApiSecret, (req, res) => {
    try {
        const engines = listEngineCredentials(engineRegistry);
        res.json({
            success: true, bot: 'stock', broker: 'alpaca', count: engines.length,
            failing: engines.filter(e => e.credentialsValid === false || e.demo).length,
            engines,
        });
    } catch (e) {
        console.error('[admin/engine-credentials] stock:', e.message);
        res.status(500).json({ success: false, error: 'Internal server error' });
    }
});
...
app.post('/api/notify', requireApiSecret, async (req, res) => {
    const title = (req.body?.title || '').toString().slice(0, 200);
    const body = (req.body?.body || '').toString().slice(0, 3500);
    if (!title && !body) {
        return res.status(400).json({ success: false, error: 'title or body required' });
    }
    const token = process.env.TELEGRAM_BOT_TOKEN;
    const chatId = process.env.TELEGRAM_CHAT_ID;
    if (!token || !chatId) {
        return res.status(503).json({ success: false, error: 'Telegram not configured' });
    }
    ...
    try {
        ...
        console.warn('[NOTIFY] Telegram send failed:', err?.response?.data || err.message);
        res.status(502).json({ success: false, error: 'Telegram delivery failed' });
    }
});
