...
async function managePositions() {
    ...
    for (const oandaPos of oandaPositions) {
        ...
        const longUnits = parseInt(oandaPos.long?.units || '0');
        const shortUnits = parseInt(oandaPos.short?.units || '0');
        const isLong = longUnits > 0;
        const units = Math.abs(longUnits) || Math.abs(shortUnits);
        ...
        // Enforce computational stop loss (since we trail computationally to save API calls)
        if (localPos.direction === 'long' && currentPrice <= localPos.stopLoss) {
            await closePositionWithReason(pair, `Trailing stop hit at ${currentPrice.toFixed(5)}`);
            continue;
        } else if (localPos.direction === 'short' && currentPrice >= localPos.stopLoss) {
            await closePositionWithReason(pair, `Trailing stop hit at ${currentPrice.toFixed(5)}`);
            continue;
        }
...
app.get('/api/forex/status', async (req, res) => {
    try {
        ...
        const positionsData = oandaPositions.map(pos => {
            const longUnits = parseInt(pos.long?.units || '0');
            const shortUnits = parseInt(pos.short?.units || '0');
            const isLong = longUnits > 0;
            const units = Math.abs(longUnits) || Math.abs(shortUnits);
