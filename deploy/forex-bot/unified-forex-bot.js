...
app.get('/api/admin/engine-credentials', requireApiSecret, (req, res) => {
    try {
        const engines = listEngineCredentials(forexEngineRegistry);
        res.json({
            success: true, bot: 'forex', broker: 'oanda', count: engines.length,
            failing: engines.filter(e => e.credentialsValid === false || e.demo).length,
            engines,
        });
    } catch (e) {
        console.error('[admin/engine-credentials] forex:', e.message);
