...
MAX_DAILY_LOSS_PCT = -3.0  # portfolio-level daily loss circuit breaker
...
class PortfolioRiskAgent:
    ...
    async def check_risk(
        ...
        result.daily_pnl_pct = snapshot['daily_pnl']
        ...
        if snapshot['daily_pnl'] < MAX_DAILY_LOSS_PCT:
            result.risk_flags.append(
                f"BLOCK: daily_loss_limit (${snapshot['daily_pnl']:.2f} < ${MAX_DAILY_LOSS_PCT})"
            )
